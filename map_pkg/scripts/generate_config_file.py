...
class ObstacleConfig:
    ...
    def random(self, map_config: MapConfig, shelfini: list, gates: list, obstacles: list, no_boxes=False,
               no_cylinders=False, max_size=1, min_size=0.5) -> None:
    ...
    def __random_config(self, map_config: MapConfig, no_boxes=False, no_cylinders=False, max_size=1,
                        min_size=0.5) -> None:
        if no_boxes:
            self.type = "cylinder"
        elif no_cylinders:
...
class VictimConfig:
    ...
    def random(self, map_config: MapConfig, shelfini: list, gates: list, obstacles: list, min_weight=0.5,
               max_weight=1) -> None:
...
def generate_config_file(map_env_params_file, generated_config_file):
    ...
    if not os.path.exists(Path(generated_config_file).parent):
        raise Exception(
            f"The parent directory of the output file `{Path(generated_config_file).parent}` does not exist, hence cannot create")
    ...
    for i in range(len(gates_yaml['x'])):
        ...
        if gates_yaml['x'][i] == 0 and gates_yaml['y'][i] == 0:
            ...
            assert gate.check(map_config=map_config, shelfini=shelfini,
                              gates=gates), f"{gate} is not in the map or intersects with another gate"
        gates.append(gate)

    # Generate obstacles config
    obstacles_yaml = config_yaml['/**/send_obstacles']['ros__parameters']
    obstacles = []
    assert len(obstacles_yaml['vect_x']) == len(obstacles_yaml['vect_y']) == len(obstacles_yaml['vect_dim_x']) == len(
        obstacles_yaml['vect_dim_y']) == len(obstacles_yaml['vect_yaw']) == len(obstacles_yaml['vect_type'])
    for i in range(len(obstacles_yaml['vect_x'])):
        ...
        assert obstacle.check(map_config, shelfini, gates,
                              obstacles), f"{obstacle} is not in the map or intersects with another gate or obstacle"
    ...
    if config_yaml['/**/send_victims']['ros__parameters']['victims_activated']:
        ...
        for i in range(len(victims_yaml['vect_x'])):
            ...
            assert victim.check(map_config, shelfini, gates,
                                obstacles), f"{victim} is not in the map {map_config} or intersects with another gate or obstacle"
    ...
    # Set gates info
    full_conf_file["/_/send_gates"] = {
        "ros__parameters": {}
    }
    full_conf_file["/_/send_gates"]["ros__parameters"]["x"] = [float(gate.x) for gate in gates]
    full_conf_file["/_/send_gates"]["ros__parameters"]["y"] = [float(gate.y) for gate in gates]
    full_conf_file["/_/send_gates"]["ros__parameters"]["yaw"] = [float(gate.yaw) for gate in gates]

    # Set obstacles info
    full_conf_file["/_/send_obstacles"] = {
        "ros__parameters": {}
    }

    full_conf_file["/_/send_obstacles"]["ros__parameters"]["n_obstacles"] = obstacles_yaml['n_obstacles']
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["no_cylinders"] = obstacles_yaml['no_cylinders']
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["max_size"] = obstacles_yaml['max_size']
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["min_size"] = obstacles_yaml['min_size']
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["vect_type"] = [str(obstacle.type) for obstacle in
                                                                            obstacles]
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["vect_x"] = [float(obstacle.x) for obstacle in obstacles]
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["vect_y"] = [float(obstacle.y) for obstacle in obstacles]
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["vect_yaw"] = [float(obstacle.yaw) for obstacle in
                                                                           obstacles]
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["vect_dim_x"] = [float(obstacle.dx) for obstacle in
                                                                             obstacles]
    full_conf_file["/_/send_obstacles"]["ros__parameters"]["vect_dim_y"] = [float(obstacle.dy) for obstacle in
                                                                             obstacles]

    # Set victims info
    full_conf_file["/_/send_victims"] = {
        "ros__parameters": {}
    }
    full_conf_file["/_/send_victims"]["ros__parameters"]["victims_activated"] = \
    config_yaml['/**/send_victims']['ros__parameters']['victims_activated']
    if config_yaml['/**/send_victims']['ros__parameters']['victims_activated']:
        full_conf_file["/_/send_victims"]["ros__parameters"]["max_weight"] = max_weight
        full_conf_file["/_/send_victims"]["ros__parameters"]["min_weight"] = min_weight
        full_conf_file["/_/send_victims"]["ros__parameters"]["vect_x"] = [float(victim.x) for victim in victims]
        full_conf_file["/_/send_victims"]["ros__parameters"]["vect_y"] = [float(victim.y) for victim in victims]
        full_conf_file["/_/send_victims"]["ros__parameters"]["vect_weight"] = [float(victim.weight) for victim in
                                                                                victims]
        full_conf_file["/_/send_victims"]["ros__parameters"]["victims_timeout"] = victims_yaml['victims_timeout']
    # Set timeout info
    full_conf_file["/_/send_timeout"] = {
        "ros__parameters": {
    ...
    if os.path.exists(generated_config_file):
        print(colored(f"generate_config_file.py:  remove previous {generated_config_file}", "red"))
        os.remove(generated_config_file)

    # this    creates     install / share / map_pkg / config / full_config.yaml overwriting, thanks to flag w, any previous instance
    print(colored(f"generate_config_file.py:  saving new full_config.yaml", "red"))
    yaml.dump(full_conf_file, open(generated_config_file, "w"))
    print(colored(f"generate_config_file.py:  loading full_config.yaml in param server", "red"))
    # upload full_config.yaml  into param server
    for params, ns_in_file in rosparam.load_file(generated_config_file):
        rosparam.upload_params("/", params)  # forces root, ignores ns in file



def Configurator():
    rospy.init_node('configurator')
    import rospkg
    default_config = rospkg.RosPack().get_path('map_pkg')+ 'config'

    map_env_params_file = rospy.get_param('~map_env_params_file', os.path.join(default_config, 'map_config.yaml'))
    gen_map_params_file = rospy.get_param('~gen_map_params_file', os.path.join(default_config, 'full_config.yaml'))
    generate_new_config = rospy.get_param('~generate_new_config', True)

    rospy.loginfo(f"map_env_params_file: {map_env_params_file}")
    rospy.loginfo(f"gen_map_params_file: {gen_map_params_file}")
    rospy.loginfo(f"generate_new_config: {generate_new_config}")

    if generate_new_config:
        generate_config_file(map_env_params_file, gen_map_params_file)
        rospy.loginfo(f"Config file {gen_map_params_file} generated.")

    else:
        if not os.path.exists(gen_map_params_file):
            rospy.logwarn("Config file does not exist and generation is disabled.")
        else:
            rospy.loginfo("Config file already exists.")
