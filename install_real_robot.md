...
2. check the ip of the wifi board on both HOSTCOMPUTER and LIMO

```
ifconfig
```

3. add to /etc/hosts of both HOSTCOMPUTER and LIMO (password agx) the respective IPs
...
--volume="$XAUTHORITY:$XAUTHORITY:rw" --volume $HOME/trento_lab_home:$HOME --env=HOME --env=USER  --privileged  -e SHELL --env="DISPLAY=$DISPLAY" --shm-size 2g --rm  --entrypoint /bin/bash loconav_robot'
